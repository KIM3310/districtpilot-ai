...
-- 2d. (Skipped — Model B is now Y-only, no exogenous data needed)
-- CREATE OR REPLACE TABLE FORECAST_INPUT_FUTURE_B AS
CREATE OR REPLACE TABLE FORECAST_INPUT_FUTURE_B AS
SELECT DS, DISTRICT FROM (SELECT NULL AS DS, NULL AS DISTRICT) WHERE 1=0;
-- Original future exogenous CTE (commented out for Y-only):
/*
...
-- 2e. Generate forecast (Y-only, no exogenous)
...
-- are normalized to the documented Snowflake columns so downstream SQL
-- and the Streamlit app read the same schema.
...
SELECT
    ...
    SERIES,
    MAX(CASE WHEN UPPER(ERROR_METRIC) = 'MAPE'  THEN METRIC_VALUE END) AS MAPE,
    MAX(CASE WHEN UPPER(ERROR_METRIC) = 'SMAPE' THEN METRIC_VALUE END) AS SMAPE,
    MAX(CASE WHEN UPPER(ERROR_METRIC) = 'MAE'   THEN METRIC_VALUE END) AS MAE
FROM EVAL_METRICS_A
GROUP BY SERIES;
